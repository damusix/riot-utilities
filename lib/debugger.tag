<debugger>
    ...
    <section id='actions'>
        ...
        <debugitem each={ actions }></debugitem>
    ...
    <style>
        ...
        :scope #actions debugitem:hover {

            border-color: #f70;
            box-shadow: 0px 10px 5px -8px rgba(0,0,0,0.25);
        }

        :scope #actions debugitem code {
...
<debugitem>
    ...
    <span class="name" if={ obj && obj.name }>
        { obj.name }
    </span>
    <b>{ fn }</b>
    &mdash;
    ...
    <p>Arguments</p>
    <div each={ arg in args }>

        <i>{ arg.constructor.name }</i>
        &mdash;

        <span if={ ['object', 'function'].indexOf(typeof arg) === -1 }>{ arg }</span>
        <code if={ typeof arg === 'object' }>{ JSON.stringify(arg) }</code>
        <code if={ typeof arg === 'function' }>{ arg }</code>
    </div>
